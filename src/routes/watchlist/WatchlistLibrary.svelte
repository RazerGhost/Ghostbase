<script lang="ts">
    /**
     * The watchlist itself, once Simkl has answered. Split out of the page so
     * the page can render its header straight away and stream this in (see
     * src/lib/server/stream.ts) — everything here is derived from the
     * library, and derived state cannot live inside an {#await}.
     *
     * The media-type toggle stays in the page's header row, so the group and
    ...
    import type { LoadedLibrary } from "./+page.server";
    import type { Group } from "./groups";

    let {
        library,
        activeGroup = $bindable(),
    ...
    function inGroup(item: LibraryItem, group: Group): boolean {
        return group === "anime"
    ...
    function surpriseMe() {
        const pool = library.planToWatch.filter(
    ...
    const groupWatching = $derived(
        library.watching.filter((item) => inGroup(item, activeGroup)),
    );
    const groupOnHold = $derived(
        library.onHold.filter((item) => inGroup(item, activeGroup)),
    );
    const groupCompleted = $derived(
        library.completed.filter((item) => inGroup(item, activeGroup)),
    );
    const groupDropped = $derived(
        library.dropped.filter((item) => inGroup(item, activeGroup)),
    );
    const groupPlanToWatch = $derived(
        library.planToWatch.filter((item) => inGroup(item, activeGroup)),
    );
    const groupSurprisePool = $derived(
        library.planToWatch.filter((item) => item.mediaType !== "anime"),
    );
