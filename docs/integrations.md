...
[SpotifyWidget.svelte](../src/lib/components/SpotifyWidget.svelte) (the live line in the running head) and [ListeningNowCard.svelte](../src/lib/components/ListeningNowCard.svelte) (the homepage card) both get their live currently-playing state from [lanyard.svelte.ts](../src/lib/stores/lanyard.svelte.ts) — Lanyard's Spotify field, not Spotify's own `currently-playing` API. That used to be a real `/api/spotify` route backed by Spotify's API, polled independently by whichever of these components were mounted on a given page — e.g. the homepage renders `ListeningNowCard` *and* `DiscordPresence` together, and `SpotifyWidget` is mounted globally in the running head on top of that — with the currently-playing poll tightening down to every 4s near the end of a track. Since every visitor's poll went through the site's one shared Spotify token, that was enough concurrent traffic to trip Spotify's rate limit and put the account on a cooldown. Lanyard gets its Spotify data from Discord's Rich Presence gateway (pushed, not polled from Spotify), so reading from there instead removes Spotify's API from this path entirely.

**How updates arrive**: the store holds one WebSocket to Lanyard (`wss://api.lanyard.rest/socket`), which pushes each presence change as Discord reports it — a track change shows within a second or two rather than waiting out a poll interval. If the socket can't connect (or hasn't delivered its initial state within 5s) the store falls back to polling Lanyard's REST endpoint every 20s, reconnecting the socket with exponential backoff (1s → 60s) and dropping the polling the moment it's back. Returning to a backgrounded tab reconnects immediately, since throttled timers can starve the heartbeat.

**Stale tracks**: components read the track through `spotifyAt(now)`, backed by `liveSpotify()` in [lanyard-live.ts](../src/lib/stores/lanyard-live.ts), rather than Lanyard's raw `spotify` field. It hides a track once it's more than 15s past its own end time (Discord has stopped pushing — client closed, sharing dropped), and when the socket is down and Lanyard hasn't answered for over 60s. Before this, a dropped presence or a failing Lanyard left the last track on screen as if it were still playing.
...
[DiscordPresence.svelte](../src/lib/components/DiscordPresence.svelte), and now the Spotify widgets above, all read from the shared [lanyard.svelte.ts](../src/lib/stores/lanyard.svelte.ts) store — a single reference-counted connection so multiple components on the same page (e.g. the homepage's compact Discord card plus the Spotify line in the running head) don't each open their own against [Lanyard](https://github.com/Phineas/lanyard), a free third-party service that surfaces a Discord user's live presence via their public Discord ID (not a secret — Discord IDs are public). Configured via `discordUserId` in [config.ts](../src/lib/config.ts).
